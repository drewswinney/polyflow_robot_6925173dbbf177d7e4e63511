{ config, pkgs, lib, webrtcPkg, pyEnv, webrtcEnv, robotConsoleStatic, robotApiPkg, ... }:

let
  user      = "admin";
  password  = "password";
  hostname  = "6925173dbbf177d7e4e63511";
  homeDir   = "/home/${user}";
  ...
  wifiModeSwitch = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      if [ -z "$ROBOT_UUID" ]; then
        nmcli connection add type wifi ifname "$WIFI_IF" con-name robot-wifi ssid "$WIFI_SSID"
        # Capture UUID for the newly created connection so we never rely on NAME
        ROBOT_UUID="$(nmcli -t -f UUID,NAME connection show \
          | awk -F: '$2=="robot-wifi"{print $1; exit}')"
      else
        nmcli connection modify "$ROBOT_UUID" 802-11-wireless.ssid "$WIFI_SSID"
      fi
      ...
      if [ -n "''${WIFI_PSK:-}" ]; then
        ...
        nmcli connection modify "$ROBOT_UUID" 802-11-wireless-security.key-mgmt none
      ...
      nmcli connection modify robot-ap connection.autoconnect no || true
...
in
{
  ...
  services.loki = {
    ...
    configuration = {
      ...
      common = {
        ...
        # IMPORTANT: must be a *private* interface in 3.4
        instance_interface_names = [ "dummy0" ];

        ring = {
          kvstore.store = "inmemory";
          instance_addr = "10.254.254.1";  # stable private advertise addr
        };
      };
      ...
      ingester = {
        wal.enabled = false;
        lifecycler = {
          address = "10.254.254.1"; # stable private advertise addr
          join_after = "0s";
          final_sleep = "0s";
        };
      };

      # Workaround for 3.4.x bug: memberlist-kv initializes anyway.
      # Give it an explicit usable advertise/bind address so it doesn't die.
      memberlist = {
        ...
        # Single node: don't try to join anything.
        join_members = [ ];
  ...
  systemd.services.loki.after = lib.mkAfter [
    "network-online.target"
    "polyflow-wifi-mode.service"
  ];

  systemd.services.loki.wants = lib.mkAfter [
    "network-online.target"
  ];
